...
// 변수하나 안에 한개이상의 키와 값이 조합되 데이터를 넣어 사용합니다
// 객체안에 있는 키와 값의 조합 하나를 속성이라고 하며, 각 속성은 콤마(,)로 구분합니다

// 객체내의 키를 이용한 값의 출력
console.log(product['제조사']);
console.log(product.name);

// 자바스크립트의 객체는 별도의 클래스 선언 없이, { }중괄호 안에 직접 속서들을 넣는 순간 객체(Object)로 인식되어 사용되어 집니다.
...
// 멤버 함수가 멤버변수로의 접근
// - this 키워드 : 자바스크립트는 멤버변수에 접근을 위해서 반드시 this 키워드를 써야 합니다
var person = {
    name:'홍길동',
    eat:function(food){
...
// 함수안에   this  를 이용한 변수에 값을 넣으면 그 이름의 멤버 변수가 만들어지고, 최종 그 변수들을 멤버로하는 객체가 만들어지는 생성자 함수로 인식됩니다.
...
// 그 함수로 만들어질 객체를 위한  프로토타입이라고 하는 객체가 생성됩니다.
// 프로토타입은  생성될 객체의 원본이며, 
// 프로토타입 또한 객체형태로 존재합니다.

var std1 = new Student('홍길서',87,98,87,45);
// 위명령이 실행되는 순간 프로토타입의 사본이  std1에 저장되면서 새로운객체를 이룹니다
...
console.log();
// 새로 추가된 멤버변수 basicLanguage 변수의 값을 전달인수로 전달해서 초기화할수는 없습니다
std1 = new  Student('홍길서', 95,87,98,99);  // basicLanguage 의 값은 100입니다
console.log( std1.toString() );
// 프로토타입은 생성자 안에서 새로만들어지는 객체에 복사되기 위해 준비되고 있는 공간 
// 그안에 새로만들어질 객체의 모습을 갖춘 객체입니다.
// 생성자에 멤버변수와 멤버메서드를 추가하려면 반드시 이 프로토타입을 이용하세요~


// 객체를 생성후에 멤버메서드를 추가하느냐, 프로토타입에 메서드추가후 객체를 만드느냐는 
// 선택적으로 사용할수 있습니다
