...
app.use('/img', express.static(path.join(__dirname, 'uploads'))); //이미지용 static 폴더 별도 생성
...
})); // 한명의 사용자(클라이언트)에 의해 세션에 한개이상 값이 저장되면 그값을 사용하는 클라이언트를 구분할수 있는 쿠키가 자동으로 클라이언트에 심어집니다, 그 쿠키에 대한 설정입니다. 세션값이 삭제되거나 브라우저가 닫히면 쿠키값도 사라집니다.
...
// passport 모듈의 사용은 세션과 연관이 있으므로 그뒤에 미들웨어를 설정합니다.
...
// app.get('/', (req, res)=>{  res.send('<h1>Node Gram</h1>');});
